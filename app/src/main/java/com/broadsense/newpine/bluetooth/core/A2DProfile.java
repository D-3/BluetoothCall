...
public class A2DProfile extends StateMachine implements Profile {
    private static final String TAG = "A2DProfile";
    ...
    private StreamingState      mStreamingState     = new StreamingState();
    private boolean isRegisted = false;

    private BluetoothDevice mBluetoothDevice;

    public A2DProfile(BluetoothDevice device){
        super("A2DProfileState");
    ...
    public boolean isConnected() {
        return getCurrentState() == connectedState
                || getCurrentState() == mStreamingState;
    }
    ...
    public void connect(){
        ...
        //发送连接命令
//        sendMessage(obtainMessage(0, "AT+B A2DPCONN " + mBluetoothDevice.getAddress()));
    }
    ...
    public void onEvent(String response) {
        ...
        //断开连接时取消监听
        if (response.contains("AT-B A2DPSTAT 1")) {
    ...
    private class ConnectingState extends State {
        ...
        public boolean processMessage(Message msg) {
            if(msg != null){
                ...
                //已连接
                if(response.contains("AT-B A2DPSTAT 3")){
                    transitionTo(connectedState);
                }
                //断开连接
                else if(response.contains("AT-B A2DPSTAT 1")){
    ...
    private class ConnectedState extends State {
        ...
        public boolean processMessage(Message msg) {
            if(msg != null){
                ...
                //播放中
                if(response.contains("AT-B A2DPSTAT 4")){
                    transitionTo(connectedState);
                }
                //断开连接
                else if(response.contains("AT-B A2DPSTAT 1")){
